...
- **Editor del carrito + Carritos abandonados (2026-09-25, Thiago)**: `shared/bundle/cart.js` es la fuente única del drawer: `CART_TEXT_DEFAULTS` / `CART_TOGGLE_DEFAULTS`, `sanitizeCartSettings` / `resolveCartSettings(saved, checkoutTheme)` y las funciones que viajan al navegador `cartCss / cartShellHtml / cartBodyHtml / cartCtaText` (api/widget.js las inyecta con `fnSrc = f.toString()` dentro del template literal: lo interpolado con `${}` entra tal cual, NO duplicar barras). Se guarda parcial en `merchants.cart_settings` (`save-settings`; `{}` = default; un texto vaciado se guarda como "x" = apagado) y el panel **Catálogo → Carrito** (`CartEditor` en CheckoutDesigner.jsx) edita textos ({{qty}} {{freq}} {{total}}), qué se muestra (regalos, tachado, ahorro, envío, nota, pie) y colores (por defecto los del checkout; `use_checkout_theme:false` + color/bg/text) con vista previa que usa el MISMO render. **Carritos abandonados** es pestaña propia (Ventas, entre Suscripciones y Cobros; `SubscriptionsPage carts`): los checkouts sin pagar de 30 días; en Suscripciones ya no está el estado "Sin pagar"; el alias `abandonados` y Retención llevan ahí.
- **La venta pasa a ser demo + puesta en marcha paga (2026-09-25, Thiago)**: el autoservicio no cerraba. Las dos tiendas que funcionan (Glow Derm y Well Fresh) salieron las dos con Thiago integrando a mano, y los 4 leads del primer creativo de Meta se registraron solos, contestaron 3 de 4 "sin ventas" y ninguno conectó tienda. Ahora: (1) **`#/demo`** (`src/pages/Demo.jsx`, sin cuenta) es el CTA principal de la landing — marca, pedidos POR DÍA, recurrencia actual, objetivo, y **dos casillas obligatorias**: que se presenta a la llamada y que acepta pagar **USD 100** cuando la integración esté terminada y funcionando. Preguntas y validación en `shared/platform/demoLead.js` (la MISMA `sanitizeDemoLead` corre en el navegador y en `api/public.js?action=demo-lead`); el lead va a `demo_leads/{id}` (cerrado en `firestore.rules`), avisa por `notifyAdmin` y responde ok aunque el aviso falle. Rate limit 10/h por IP. (2) **El registro self-serve sigue** en `#/registro` como segunda opción (lo necesita la instalación desde la app store de Tiendanube). (3) **`INSTALL_USD` pasa de null a 100** en `shared/platform/pricing.js` (fuente única; Integrations.jsx lo tenía duplicado a mano) y **no queda un solo lugar que diga que la instalación es gratis**. (4) **Fuera los videos**: `SHOPIFY_TUTORIAL_URL` y `LANDING_VIDEOS` vacíos (los archivos siguen en el bucket y en `public/landing/`) porque quedaron viejos y repiten que todo es gratis; `TutorialVideo` ya no devuelve null sin URL, así el botón de WhatsApp de abajo —ahora el camino principal— no desaparece. (5) **Pauta**: evento propio **`RegistroCalificado`** al pixel de Recurrentes; sale del formulario (`trackQualifiedLead`) y del registro cuando vende 50+ pedidos/mes o pide la instalación (`leadCalifica`). `registered` → CompleteRegistration sigue como métrica aparte. Admin → Adquisición tiene la columna "Registros calificados"; los pedidos de demo todavía NO aparecen en esa tabla (solo por el aviso). Tests: `tests/money-path/demo-lead.test.mjs` y `acquisition.test.mjs`.
