// Firestore en memoria (subset de la API del Admin SDK que usa /api).
// __failGet: paths de docs cuyo get() lanza (para probar manejo de errores).
const store = new Map();
export const __store = store;
export const __failGet = new Set();
let autoN = 0;
...
function applyVal(obj, key, v) {
  ...
  if (v && typeof v === "object" && !Array.isArray(v) && typeof v.toMillis !== "function") {
  ...
  obj[key] = typeof v?.toMillis === "function" ? v.toMillis() : clone(v);
}
...
class DocRef {
  ...
  async get() {
    if (__failGet.has(this.path)) throw new Error(`FAILGET ${this.path} internal detail`);
    return snap(this, store.get(this.path));
  }
  async set(data, opts = {}) {
...
class Query {
  ...
  async get() { const docs = this._docs().map(([p, d]) => snap(new DocRef(p), d)); return { docs, empty: !docs.length, size: docs.length, forEach: (fn) => docs.forEach(fn) }; }
