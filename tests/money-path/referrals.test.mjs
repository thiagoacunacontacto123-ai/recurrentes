...
test("(r) comisión: 15% del precio de lista del tramo, una vez por pago, y va al saldo de Stripe si hay customer", async () => {
  ...
  assert.equal(r.usd, 7.35, "49 × 15%");
  assert.equal(r.pushed, 7.35);
  const ref = rawGet("merchants/ref_uid");
  assert.equal(ref.ref_earned_usd, 7.35);
  assert.equal(ref.ref_credit_pending_usd, 0);
  assert.equal(ref.ref_credit_applied_usd, 7.35);
  assert.equal(stripeCalls.length, 1);
  assert.equal(stripeCalls[0].path, "/v1/customers/cus_ref/balance_transactions");
  assert.equal(stripeCalls[0].params.amount, -735, "saldo a favor en centavos");
  ...
  assert.equal(rawGet("merchants/ref_uid").ref_earned_usd, 7.35);
  assert.equal(stripeCalls.length, 1);
});
...
test("(r) sin customer en Stripe el crédito queda pendiente y se empuja al activar el plan", async () => {
  ...
  assert.equal(r.usd, 14.85, "99 × 15%");
  assert.equal(r.pushed, 0);
  assert.equal(rawGet("merchants/ref_uid").ref_credit_pending_usd, 14.85);
  ...
  assert.equal(p.pushed, 14.85);
  assert.equal(stripeCalls[0].params.amount, -1485);
