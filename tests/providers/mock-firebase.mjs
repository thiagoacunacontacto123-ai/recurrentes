...
// Transacciones SERIALIZADAS (como Firestore): permite probar dos webhooks a la vez.
let txQueue = Promise.resolve();
const _db = {
  collection: (n) => new ColRef(n),
  runTransaction: (fn) => {
    const run = txQueue.then(() => fn({ get: (r) => r.get(), set: (r, d, o) => r.set(d, o), update: (r, d) => r.update(d), create: (r, d) => r.create(d) }));
    txQueue = run.catch(() => {});
    return run;
  },
};
