...
// ── 1) Quién manda: número propio > número de Recurrentes > nadie ──
{
  ...
  ok(near(safe.whatsapp_charge_usd, 0.0132) && P.WHATSAPP_MARKUP === 1.1, `precio por aviso con 10% (${safe.whatsapp_charge_usd})`);
}
...
let firstWamid;
{
  ...
  ok(u.wa_sent === 1 && u.wa_platform_sent === 1 && near(u.wa_cost_usd, 0.012 * 1.1), `uso del mes de la tienda: 1 aviso, US$ ${u.wa_cost_usd} (precio × 1,10)`);
  ok(au.merchants.mp1.wa_sent === 1 && near(au.merchants.mp1.wa_cost_usd, 0.0132) && near(au.merchants.mp1.wa_meta_cost_usd, 0.012) && au.wa_sent === 1, "admin: uso por comercio y total del mes");
  ...
  ok(u2.wa_sent === 2 && near(u2.wa_cost_usd, 0.0132 + 0.022), `WHATSAPP_PRICE_USD_UTILITY pisa el precio (acumulado US$ ${u2.wa_cost_usd})`);
  ...
  ok(usage.usage?.wa_sent === 2 && usage.sender === "platform" && near(usage.charge_usd, 0.0132), "GET whatsapp-usage para el panel");
}
